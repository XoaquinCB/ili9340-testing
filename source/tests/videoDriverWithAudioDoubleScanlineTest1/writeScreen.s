...
writeScreen:
    ...
    ;############################################### Initialise variables ##############################################
        ...
        lds YL, nextAudioSamplePtr                     ; (H) Load address of next audio sample into Y register (low byte)
        lds YH, nextAudioSamplePtr+1                   ; (H) Load address of next audio sample into Y register (high byte)
        sbiw YL, 1                                     ; (H-H) Convert to current sample (instead of next sample)
    ...
    ;############################## Write next column until all columns have been written ##############################
        ...
        ; If column counter has reached 13 or 8, generate the second VSYNC pulse:
    #if TILE_WIDTH == 6
        cpi r22, 14                                    ; (H)
    #elif TILE_WIDTH == 8
        cpi r22, 8                                     ; (H)
    ...
    ;#################################################### Finish up ####################################################
        ...
        adiw YL, 1                                     ; (H-H)
        sts nextAudioSamplePtr, YL                     ; (H)
        sts nextAudioSamplePtr+1, YH                   ; (H)
